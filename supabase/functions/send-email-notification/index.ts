...
Deno.serve(async (req: Request) => {
  ...
  if (!payload.project_id || !payload.type || !["project_confirmed", "project_status_changed"].includes(payload.type)) return json({ success: false, error: "Unsupported notification" }, 400);
  if (payload.type === "project_status_changed" && (!payload.previous_status || !payload.new_status)) return json({ success: false, error: "Status transition is required" }, 400);
  ...
  if (projectError || !project) return json({ success: false, error: "Project not found" }, 404);
  const isOwner = project.client_id === authData.user.id;
  const isActiveAdmin = callerProfile?.role === "admin" && callerProfile?.status === "active";
  if (!isOwner && !isActiveAdmin) return json({ success: false, error: "Forbidden" }, 403);
  ...
  try {
    ...
    if (!response.ok || result.success !== true) { console.error("Transactional email delivery failed", { status: response.status, projectId: project.id, type: payload.type }); return json({ success: false, error: "Email delivery failed" }, 502); }
    return json({ success: true });
  } catch (error) {
    console.error("Transactional email request failed", { projectId: project.id, type: payload.type, message: error instanceof Error ? error.message : "unknown" });
