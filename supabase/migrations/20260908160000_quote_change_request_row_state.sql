-- Quote change-request row state + admin visibility (additive).
--
-- Why: Accept (accepted_at) and Reject (rejected_at) each leave a durable
-- marker on the quote row that the Admin Panel already renders. "Request
-- changes" only wrote an audit_logs row / a project_messages row, so it was
-- effectively invisible everywhere except the raw audit stream.
--
-- This migration:
--   1. Adds client_change_requested_at / client_change_message to quotes, so a
--      change request is as visible as accept/reject on every quotes surface.
--   2. Re-creates client_respond_to_quote so the request_changes and reject
--      paths also resolve an existing project through the request (PR -> PJ)
--      before writing project_messages / audit_logs. Previously a reject or
--      change request on a request that already had a project (created by an
--      earlier accepted quote version) was only logged against the null
--      quote.project_id and never reached the project message thread.
--
-- * Additive schema change only (two nullable columns).
-- * Preserves all existing accept / reject / versioning / project-creation
--   behavior verbatim from 20260908150000.
-- * No new table, no parallel notification system: the Admin Panel continues
--   to read quote responses from audit_logs (activity stream) + quotes.
